...
export let options: Partial<Options> = {
  ...
  thresholds: {
    http_req_duration: ["avg<500", "p(95)<15000"],
...
export default (_authToken: string) => {
  ...
  group("Create and Modify Crocs", () => {
    ...
    URL = crocodileOwnerActions.createCrocodile(
      requestConfigWithTag,
      URL,
      numberOfCrocodilesCreated
    );

    crocodileOwnerActions.updateCrocodile(
      requestConfigWithTag,
      URL,
      "New Name",
      numberOfCrocodilesUpdated
    );

    crocodileOwnerActions.deleteCrocodile(
      requestConfigWithTag,
      URL,
      numberOfCrocodilesDeleted
    );
  });
